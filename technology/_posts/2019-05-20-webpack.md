...
# 学习webpack打包工具
## webpack核心概念
1. Entry 入口文件，构建其内部 依赖图(dependency graph) 的开始
1. Output 输出文件，告诉 webpack 在哪里输出它所创建的 bundle，以及如何命名这些文件
...
1. plugin 插件的范围包括，从打包优化和压缩，一直到重新定义环境中的变量
1. Mode 模式，通过选择 development, production 或 none 之中的一个，来设置 mode 参数，你可以启用 webpack 内置在相应环境下的优化。其默认值为 production
1. Browser Compatibility 浏览器兼容性，想要支持旧版本浏览器，在使用这些表达式之前，还需要 提前加载 polyfill
1. Enviroment 运行webpack的node版本
## 开发/测试/生产环境有什么不同，需要做哪些webpack配置
开发/测试环境
- 文件依赖方式-直接引入
- 热更新
- 文件模块中的依赖路径信息
...
```
2. 在返回的webpack json文件中配置devtool属性，控制是否启用sourceMap
```js
// 控制是否生成sourceMap
...
```
3. 编写公共方法，处理css/scss/less的loaders是否启用sourceMap
```js
...
```
4. 压缩代码时，配置是否删除代码中的console.log。提取css到单独文件并压缩时，是否启用sourceMap。
```js
minimizer: [
  new TerserPlugin({
    terserOptions: {
